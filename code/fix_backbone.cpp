...
//--------------------FRANCO-------------------------------------------------------------------------------------
  ...
  fprintf(dataout,"Step\tRes\tAcc/Rej\tTemp\t\tNu_pol\t\tNu_npol\t\tD_pH\t\tD_Elec\t\tD_Self\t\tV_electro\n");
...
//------------------------------------------FRANCO------------------------------------------------------------------------
    ...
    getline(input_mc, dummyLine);
    input_mc >> freqMC >> freqOUT >> tot_steps;
    input_mc.ignore(1000, '\n');
    getline(input_mc, dummyLine);
    input_mc >> temp_ini >> temp_end >> pH;
    input_mc.ignore(1000, '\n');
    getline(input_mc, dummyLine);
    input_mc >> termph_flag >> elec_flag >> self_flag;
    input_mc.ignore(1000, '\n');
    getline(input_mc, dummyLine);
...
//-------------------------FRANCO--------------------------------------
  ...
  if (huckel_flag) {
    ...
    int termph_flag, elec_flag, self_flag;
...
int FixBackbone::mc_charge_change(double **mc_data)
{
  ...
  *(*mc_data + MC_RSD) = place_change + 1;
  ...
  if(self_flag){
    term_self = delta_self(rnd_idx,place_change,new_chrg,mc_data);
    delta_mc += term_self;
  }
  // Output writing
...
double FixBackbone::delta_self(int rnd_idx, int place_change, int new_chrg, double **mc_data){
  /**
  ...
  *(*mc_data + MC_NPOLNUM) = Nnonpol;
  ...
  if(place_change < n){
    ...
    *(*mc_data + MC_NPOLNUM) = Nnonpol;
    ...
    // Direction assignment. Signs are defined for a "charging" penalty.
    if(new_chrg == 0){
	...
	for(k=0;k<n;k++){
	  ...
	  // Non Polar list
	    ...
	    if(r<rnonpol){
	      ...
	      N_nonpol = N_nonpol + exp(-alpha_nonpol*((r-rpol)*(r-rpol)));
	    }
	  }
	}
	// Result writting
...
//-----------------------------FRANCO --------------------------------------------------------------
   ...
   */
	...
	if((ntimestep/freqMC) % freqOUT == 0){
          // MC.log file
          fprintf(dataout,"%d\t%.0f\t%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",ntimestep/freqMC,mc_data[MC_RSD],resu,temp_montecarlo,mc_data[MC_POLNUM],mc_data[MC_NPOLNUM],mc_data[MC_PH],mc_data[MC_ELEC],mc_data[MC_SELF],mc_data[MC_DH]);
          // MC.state file
